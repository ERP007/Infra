...
운영 기준 compose 파일은 `docker-compose.yml`이다. `docker-compose.server.yml`, `server-images.env`는 예전 GHCR 배포 흐름의 참고/롤백용 파일로 남아 있을 수 있다.
...
이 방식에서는 `server-images.env`를 배포 source of truth로 쓰지 않는다.
